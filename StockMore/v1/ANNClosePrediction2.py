...
learning_rate = tf.train.exponential_decay(0.5, global_step, 400, 0.98, staircase=True)
train = tf.train.GradientDescentOptimizer(learning_rate)\
                .minimize(mse, global_step=global_step)

# samples_name = [1, 3, 7, 15]
...
for name in samples_name:
    # df = pd.read_csv(r'.\data\sh.600519-%02dd.csv' % name)
    ...
    predict = df_use[df['pctChg_delay'].isna()].T
    # 非Nan行作为训练集
    # features = df_use[np.bitwise_not(df['close_delay'].isna())].T
    features = df_use[np.bitwise_not(df['pctChg_delay'].isna())].T
    ...
    # 尝试预测涨跌幅，预测目标并不归一化
    labels = np.asarray(df['pctChg_delay'].loc[np.bitwise_not(df['pctChg_delay'].isna())], dtype=np.float64)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        for i in range(train_times):
            ...
            if i % 1000 == 0:
                ...
                print("MSE: ")
                print(testmse)
                print("Learning Rate: %.10f" % sess.run(learning_rate))
                # print("Global Steps: %d" % sess.run(global_step))
        ...
        plt.legend()
        # plt.title('Predict Close Price of %s' % name)
        plt.title('Predict Price Change of %s' % name)
