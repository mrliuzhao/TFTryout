...
# final_output = active_func(final_output)
...
for name in samples_name:
    ...
    # 记录下nan的行作为预测用
    # predict = df_use[df['raise_prob'].isna()].T
    predict = df_use[df['chg_avg'].isna()].T
    # 非Nan行作为训练集
    # features = df_use[np.bitwise_not(df['raise_prob'].isna())].T
    features = df_use[np.bitwise_not(df['chg_avg'].isna())].T

    # 预测上涨概率
    # labels = np.asarray(df['raise_prob'].loc[np.bitwise_not(df['raise_prob'].isna())], dtype=np.float64)
    labels = np.asarray(df['chg_avg'].loc[np.bitwise_not(df['chg_avg'].isna())], dtype=np.float64)
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
        ...
        plt.xticks([])
        plt.xlabel('Time')
        # plt.ylabel('Raise Probability')
        plt.ylabel('Average of Price Change')
        ...
        plt.legend()
        # plt.title('Predict Raise Probability of %s' % name)
        plt.title('Predict Average Price Change of %s' % name)
